$(function () {
    ...
    var grid = new ej.grids.Grid({
        ...
        columns: [
            ...
            {
                ffield: 'orderDate', headerText: 'Project Order Date', textAlign: 'Left', width: 100,
        ...
        toolbarClick: (args) => {
            ...
            if (args.item.id === 'DetailsCustom') {
                if (grid.getSelectedRecords().length) {
                    var selectedId = grid.getSelectedRecords()[0].id;
                    window.location.href = '/ProjectOrder/Detail/' + selectedId
                } else {
                    abp.notify.error(l('SelectRecordError'));
                }
            }
                            ...
                            .then(function (confirmed) {
                                if (confirmed) {
                                    ...
                                    indo.projectOrders.projectOrder
                                        ...
                                        .then(function () {
                                            abp.notify.success(l('ConfirmSuccess'));
                                            reloadGrid();
                                            grid.hideSpinner();
                                        })
                                        .catch(function (error) {
                            ...
                            .then(function (confirmed) {
                                if (confirmed) {
                                    ...
                                    indo.projectOrders.projectOrder
                                        ...
                                        .then(function () {
                                            abp.notify.success(l('CancelSuccess'));
                                            reloadGrid();
                                            grid.hideSpinner();
                                        })
                                        .catch(function (error) {
    ...
    function reloadGrid() {
        indo.projectOrders.projectOrder.getList()
            .then(function (data) {
    ...
    $(document).on('click', '#btn-update-go-to-detail', function () {
        ...
        indo.projectOrders.projectOrder
            .update(formAsObject.projectOrder.id, formAsObject.projectOrder)
            .then(function () {
                abp.notify.success(l('UpdateSuccess'));
                window.location.href = '/ProjectOrder/Detail/' + formAsObject.projectOrder.id;
            })
            .catch(function (error) {
    ...
    $(document).on('click', '#btn-create-go-to-detail', function () {
        ...
        var formAsObject = $form.serializeFormToObject();
        indo.projectOrders.projectOrder
            .create(formAsObject.projectOrder)
            .then(function (args) {
                abp.notify.success(l('CreateSuccess'));
                window.location.href = '/ProjectOrder/Detail/' + args.id;
            })
            .fail(function (error) {
                abp.notify.error("Error: " + error.code + " " + error.message + " " + error.details);
            });
    ...
    /* Popup Modal */
    abp.modals.ProjectOrderModal = function () {
    ...
    var helpModal = new abp.ModalManager(abp.appPath + 'ProjectOrder/Help');
    var createModal = new abp.ModalManager({
        viewUrl: abp.appPath + 'ProjectOrder/Create',
        modalClass: 'ProjectOrderModal'
    });
    var updateModal = new abp.ModalManager({
        viewUrl: abp.appPath + 'ProjectOrder/Update',
        modalClass: 'ProjectOrderModal'
    });
    ...
    updateModal.onResult(function (args) {
        abp.notify.success(l('UpdateSuccess'));
        reloadGrid();
    });

    /* Popup Lookup Modal */
    var invoiceLookupModal = new abp.ModalManager(abp.appPath + 'ProjectOrder/InvoiceLookup');
    invoiceLookupModal.onOpen(function () {
        loadInvoiceLookupGrid($("#hfProjectOrderId").val());
    });
    function loadInvoiceLookupGrid(projectOrderId) {
        indo.customerInvoices.customerInvoice.getListWithTotalByProjectOrder(projectOrderId)
            .then(function (data) {
                var invoiceLookupGrid = new ej.grids.Grid({
                    ...
                    columns: [
                        ...
                        {
                            field: 'sourceDocument', headerText: 'Project Order', textAlign: 'Left', width: 150,
                            template: "<a href='/ProjectOrder/Detail/${sourceDocumentId}' target='_blank'>${sourceDocument}</a>"
                        },
                        {
